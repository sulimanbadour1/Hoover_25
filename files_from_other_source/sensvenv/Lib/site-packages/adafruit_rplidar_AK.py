...
#note b'' codes into ascii, \x is prefix for hexa 
__version__ = "1.2.10"
__repo__ = "https://github.com/adafruit/Adafruit_CircuitPython_RPLIDAR.git"

SYNC_BYTE = b"\xA5" #identificating byte of request from PC to lidar
SYNC_BYTE2 = b"\x5A" #byte sent by RPLidar for confirmation the scanning (response to \xA5 value)
...
DESCRIPTOR_LEN = 7 # [bytes] number of bytes sent by lidar as info answer for request followed by data bytes (one or more) answering the request
INFO_LEN = 20
HEALTH_LEN = 3
...
_SCAN_TYPES = (
    {"byte": b"\x20", "response": 129, "size": 5},#scan mode default
    {"byte": b"\x21", "response": 129, "size": 5}, #scan mode when fastest data scanning and control the stability of speed
    {"byte": b"\x82", "response": 130, "size": 84}, #scan mode sends data without controlling the stability of speed
)
...
class RPLidar:
    ...
    scanning = False #Is scanning running?
    descriptor_size = 0
    ...
    def __init__(
        ...
        #isinstance checks if port is string type 
        #not isinstance converts  if port is not string type into negative value

        #It is because than the condition below is not evaluated as True 
        # teherfore it continues to else-part where serial is imported

        #If isinstance is false it means  that our PC considers port name as a connection to port 
        # not just as a word
        #It means that our PC is already connected to the potr e.g. from previous activities
        #Also the serial must be therefore already imported and is not necessayry to import it again
        ...
        #If the port was not connected we just imported the "serial" library but the _seria_port is still None
        #Therefore we have to connect to the port by calling connect function

        self.connect() #connects to serial port default by initialization the object
        self.start_motor() #starts motor default by initialization the object
    ...
    def connect(self) -> None:
        ...
        if not self.is_CP: 
            #double not of isinstance  means like the original value of isinstance
            #therefore checks if port is instance of str 

            if self._serial_port is not None:
                #checks if serial port is already connected and disconnects it
                self.disconnect()
            try:
                #connects serial port and 
                # assigns variable _serial_port as instance of class serial with Serial function 
                #basically save function Serial as variable _serial_port
                #note: Serial() is for connecting to the serial port
    ...
    def disconnect(self) -> None:
        ...
        if self._serial_port is None:
            #if _serial_port is None it means there PC is not connected to it
            #therefore we don't have to do anything and can return back to program from a function
            return
        self._serial_port.close() #disconnects the PC from serial port
    ...
    def _read_descriptor(self) -> Tuple[int, bool, int]:
        ...
        #Descriptor packet is response containig organisation iformation (7 bytes in total) to request for data

        #serial port waits before reading the data, 
        # waits until all bytes (number represented by DESCRIPTOR_LEN) are received
        #function read (number_of_bytes) of object Serial reads number  of bytes represented by argument 'number_of_bytes' and then decodes them 
        #e.g _serial_port.read(5) reads 5 bytes and then decodes them
        descriptor = self._serial_port.read(DESCRIPTOR_LEN) #array of bytes, number of bytes = DESCRIPTOR_LEN
        self.log_bytes("debug", "Received descriptor:", descriptor)
        if len(descriptor) != DESCRIPTOR_LEN:
            #checks the number of collected bytes
            raise RPLidarException("Descriptor length mismatch")
        if not descriptor.startswith(SYNC_BYTE + SYNC_BYTE2):
            #checks the first two bytes of collected data
    ...
    def start(self, scan_type: int = SCAN_TYPE_NORMAL) -> None:
        ...
        cmd = _SCAN_TYPES[scan_type]["byte"] #taking command from scan_type
    ...
    def iter_measurements(
        ...
        #Repair AK Adafruit_library
        if not self.motor_running:
            self.start_motor()
        #--- repaired part finished 
    ...
    """
    def store_data(values: Tuple[bool, Optional[int], float, float]):
        #stores data in container
    def plot_in_real_time(values: Tuple[bool, Optional[int], float, float]):
        import matplotlib as plt
        angle=values[2]
        distance=values[3]
        if plt.plot(angle_container)"""

    def once_measurements(
            self, max_buf_meas: int = 500, scan_type: int = SCAN_TYPE_NORMAL
        ) -> [bool, Optional[int], float, float]:
            """Iterate over measurements. Note that consumer must be fast enough,
            otherwise data will be accumulated inside buffer and consumer will get
            data with increasing lag.
